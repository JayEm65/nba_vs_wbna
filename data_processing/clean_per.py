...
def clean_and_extract_top_per(nba_file, wnba_file, output_nba_file, output_wnba_file, per_column):
    # Load NBA PER CSV
    nba_df = pd.read_csv(nba_file)

    # Drop unnecessary columns and keep only Player and PER columns
    nba_df = nba_df[['Player', per_column]].dropna()

    # Save cleaned NBA PER data to a new CSV
    nba_df.to_csv(output_nba_file, index=False)
    print(f"Cleaned NBA {per_column} data saved to '{output_nba_file}'.")

    # Load WNBA PER CSV
    wnba_df = pd.read_csv(wnba_file)

    # Drop unnecessary columns and keep only Player and PER columns
    wnba_df = wnba_df[['Player', per_column]].dropna()

    # Save cleaned WNBA PER data to a new CSV
    wnba_df.to_csv(output_wnba_file, index=False)
    print(f"Cleaned WNBA {per_column} data saved to '{output_wnba_file}'.")
...
if __name__ == "__main__":
    ...
    clean_and_extract_top_per(nba_offensive_file, wnba_offensive_file, output_nba_offensive_file, output_wnba_offensive_file, 'O_PER')

    # Execute the cleaning and processing function for Defensive PER
    clean_and_extract_top_per(nba_defensive_file, wnba_defensive_file, output_nba_defensive_file, output_wnba_defensive_file, 'D_PER')